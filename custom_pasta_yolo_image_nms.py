...
img_to_detect = cv2.imread('images/testing/dif2.jpg')
...
# set only one class which is called curvi 
class_labels = ["curvi"]

#Declare List of colors as an array Green, Blue, Red, cyan, yellow, purple
#There is only one class therefore only one color is needed here it is green
class_colors = ["0,255,0"]
class_colors = [np.array(every_color.split(",")).astype("int") for every_color in class_colors]
class_colors = np.array(class_colors)
class_colors = np.tile(class_colors,(1,1))

# Loading the pasta curvi custom model 
